...
class Retriever:
    ...
    def __init__(
        ...
        if query_embedding_service is None:
            raise ValueError("Retriever requires a valid QueryEmbeddingService instance.")
        ...
        self.vector_weight = retriever_config.get('vector_weight', 0.85)
        self.keyword_weight = retriever_config.get('keyword_weight', 0.15)
        self.min_word_length = retriever_config.get('min_word_length', 3)
        self.german_stopwords = self._load_german_stopwords()

        logger.info(f"Retriever initialized with vector_weight={self.vector_weight}, keyword_weight={self.keyword_weight}")
    ...
    def _calculate_keyword_score(self, query: str, context: str) -> float:
        ...
        try:
            ...
            for term, query_freq in query_tf.items():
                query_importance = query_freq / len(query_terms)  # artık güvenli şekilde tanımlı
...
def create_hybrid_retriever(config: Optional[Dict] = None, query_embedding_service: QueryEmbeddingService = None) -> Retriever:
    """
    ...
    try:
        vector_db_service = VectorDatabaseService()
        retriever = Retriever(vector_db_service, query_embedding_service, config)
...
if __name__ == "__main__":
    """Example of how to use the Retriever with embedding service."""
